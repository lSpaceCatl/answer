// ==UserScript==
// @name        SpaceCat_ninja_answers_for_Sechen
...
// @downloadURL https://raw.githubusercontent.com/lSpaceCatl/answer/main/answers-script.user.js
...
class CheckBoxHint extends Hint {
    ...
    CreateHintDomBlock() {
        let answerParentBlock = this._domAnswerBlock.parentNode;
        let hintHtml = `
                    <div class="script-answers" style="padding-right: 20px; position: relative; display: none; background: rgb(0 0 (0 / 6%)); border-radius: 4px; font-size: 15px; max-height: 25px;">
                        ответы: <span title="Выбрали этот ответ" style="margin: 0px 30px;">0</span> 
                    </div>`;
        answerParentBlock.insertAdjacentHTML('beforeend', hintHtml);
        this._domHintBlock = answerParentBlock.querySelector('.script-answers');
    }
